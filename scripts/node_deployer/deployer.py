...
class deployer:
    ...
    def set_launch_args(self):
        ...
        if self.conn_type == connect_type.all_to_all:
            base = "./echo_node --data-dir={datadir}/{dir} {p2p} {rpc} --genesis-json genesis.json {acc_infos} --p2p-compress"
        else:
            base = "./echo_node --data-dir={datadir}/{dir} {p2p} {rpc} --genesis-json genesis.json {acc_infos} --config-seeds-only --p2p-compress"
        for i in range(self.node_count):
