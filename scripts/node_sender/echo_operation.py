...
class EchoOperation:
    ...
    def get_balance_claim_operation(
        ...
        balance_claim_operation_props["fee"].update(
            {"amount": fee_amount, "asset_id": fee_asset_id}
        )
        ...
        balance_claim_operation_props["total_claimed"].update(
            {"amount": value_amount, "asset_id": value_asset_id}
        )
        if signer is None:
            return [
                operation_id,
                balance_claim_operation_props,
    ...
    def get_transfer_operation(
        ...
        transfer_props.update(
            {"from": from_account_id, "to": to_account_id, "extensions": extensions}
        )
    ...
    def sign(self, tx, chain_id, dynamic_global_chain_data):
        ...
        little_endian = bytearray(
            decode(dynamic_global_chain_data["head_block_id"], "hex")[4:8]
        )
        ...
        tx._signatures = list(
            map(
                lambda signer: tx._crypto.sign_message(
                    chain_buffer + transaction_buffer, signer
                ).hex(),
                tx._signers,
            )
        )

    def get_sign_transaction(
        self,
        echo,
        list_operations,
        chain_id,
        dynamic_global_chain_data,
        expiration=None,
    ):
