...
def set_options(parser):
    parser.add_argument(
        ...
        dest="echo_bin",
        type=str,
        help="Path to echo_node binary, should be statically compiled!",
        required=True,
    )
    parser.add_argument(
        "-p",
    ...
    parser.add_argument(
        ...
        dest="image",
        type=str,
        help="Name of image for docker containers",
        default="",
        required=True,
    )
    parser.add_argument(
    ...
    parser.add_argument(
        ...
        "--committee_count",
        action="store",
        dest="committee_count",
        type=int,
        help="Committee count",
        default=20,
    )
    parser.add_argument(
        "-ct",
        ...
        dest="conn_type",
        type=str,
        help="Network configurations. Can be the next: all_to_all, cyclic, serial",
        default="all_to_all",
    )
    parser.add_argument(
        "-dn",
        ...
        nargs="+",
        type=int,
        help="Number on nodes which will be run under the network delay",
        default=[],
    )
    parser.add_argument(
        "-idn",
        "--inverse_delayed_node",
        dest="inverse_delayed_node",
        nargs="+",
        type=int,
        help="Number on nodes which will not be run under the network delay, all other nodes will be",
        default=[],
    )
    parser.add_argument(
        "-t",
        ...
        action="store",
        type=int,
        help="Delay on network interface in ms",
        default=0,
    )
    parser.add_argument(
        "-s",
        ...
        action="store",
        type=str,
        help="Name of test suite. Can be the next: tps, database, load, propagation ",
        required=True,
    )
    parser.add_argument(
        "-txs",
    ...
    parser.add_argument(
        ...
        action="store",
        type=str,
        help="Transaction type: transfer, create_evm, call_emv, create_x86, call_x86",
        default="transfer",
    )
    parser.add_argument(
        "-sc",
    ...
    parser.add_argument(
        ...
        "--clear",
        action="store_true",
        help="Clear containers after test execution",
    )
    ...
    if args.suite == "tps":
        if args.pumba_bin != "":
            return tps_test(
                ...
                args.txs_count,
                args.time,
                create_delayed_node_lst(args),
                get_transaction_type(args.tx_type),
                get_connection_type(args.conn_type),
                args.committee_count,
            )
        else:
            raise Exception("pumba_bin argmunet should be specified!")
    elif args.suite == "database":
        return database_size_test(
            args.node_count,
            args.echo_bin,
            args.image,
            args.committee_count,
